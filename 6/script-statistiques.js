...
// Fonction pour ajouter automatiquement la classe .example aux blocs exemples non classés
function autoTagExamples() {
    const candidates = document.querySelectorAll('.slide p, .slide div');
    candidates.forEach(el => {
        if (el.classList && el.classList.length === 0) {
            const txt = el.textContent.trim().toLowerCase();
            if (txt.startsWith('exemple') || txt.includes("à partir de l'exemple") || txt.includes('sans le savoir') || txt.includes('fred veut')) {
